...
<?php
	...
	*/	
	...
	function sql_execute_stmt($stmt, $store_result){
		if (!$stmt->execute()) {
	...
	function sql_exec_prepared_query($query, $bind_types, $bind_args, $store){
		...
		try{
			...
			$db = sql_connect_db($info['db_host'], $info['db_user'], $info['db_pw'], 
        $info['db_name']);
			
			$stmt = sql_prepare_stmt($db, $query);
			
			$tmp = call_user_func_array("mysqli_stmt_bind_param", 
        array_merge(array($stmt, $bind_types), create_ref_array($bind_args)));
			if(!$tmp)
